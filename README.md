...
| | 📚 MCP Server（本项目） | 📡 消息轮询 + 发送（本项目） | 💬 Bot 实时聊天（第三方） |
|---|---|---|---|
| **一句话** | Claude 读取你的微信聊天记录 | Claude 用你的微信号收发消息 | 在微信里跟 Claude bot 实时对话 |
| **数据来源** | 本地微信数据库 | 本地数据库 + osascript UI 自动化 | 微信 iLink Bot API |
| **方向** | 只读（查询历史） | 双向（监听 + 发送） | 双向（收 + 发） |
| **用谁的微信号** | 你自己的 | 你自己的 | 独立的 bot 账号 |
...
- 想在微信里直接跟 Claude 一对一聊天（最简单，不需要第二个号） → Bot 实时聊天
- 推荐组合：**MCP Server + Bot 实时聊天**——Claude 既能翻你的聊天历史，又能在微信里跟你实时对话
...
**MCP Server + Bot 实时聊天**（推荐）：Claude 既能翻你的聊天历史（"帮我找找上周小明说的那个链接"），又能在微信里跟你一对一实时对话。最简单的组合，不需要第二个微信号。

**MCP Server + 轮询 + 发送**：Claude 能查历史记录，还能以你的身份给任何人发消息。适合需要 Claude 帮你回复别人、在群里发言等场景。需要第二个微信号跟 Claude 对话。

**全部搭配**：Claude 能查历史、以你的身份发消息、还能在 bot 里跟你一对一聊天。

注意：MCP Server / 轮询（本项目）和 Bot 实时聊天（claude-wechat-channel）是**两个独立的项目**，安装方式不同，数据来源也不同。MCP Server 和轮询读本地数据库（你的微信账号的所有消息），Bot 实时聊天走 iLink Bot API（独立的 bot 账号）。
